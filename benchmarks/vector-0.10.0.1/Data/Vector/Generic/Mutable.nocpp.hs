...
munstreamRMax s n
  = do
      ...
      let put i x = do
                      ...
                      return i'
      i <- MStream.foldM' put n s
      return $ ((Ck.checkSlice "Data/Vector/Generic/Mutable.hs" 386) Ck.Internal) "munstreamRMax" i (n-i) n
             $ unsafeSlice i (n-i) v
...
munstreamRUnknown s
  = do
      ...
      let n = length v'
      return $ ((Ck.checkSlice "Data/Vector/Generic/Mutable.hs" 397) Ck.Internal) "unstreamRUnknown" i (n-i) n
             $ unsafeSlice i (n-i) v'
  where
    {-# INLINE [0] put #-}
...
{-# INLINE grow #-}
grow v by = ((Ck.checkLength "Data/Vector/Generic/Mutable.hs" 531) Ck.Bounds) "grow" by
          $ unsafeGrow v by
...
swap v i j = ((Ck.checkIndex "Data/Vector/Generic/Mutable.hs" 602) Ck.Bounds) "swap" i (length v)
           $ ((Ck.checkIndex "Data/Vector/Generic/Mutable.hs" 603) Ck.Bounds) "swap" j (length v)
           $ unsafeSwap v i j
...
{-# INLINE unsafeSwap #-}
unsafeSwap v i j = ((Ck.checkIndex "Data/Vector/Generic/Mutable.hs" 629) Ck.Unsafe) "unsafeSwap" i (length v)
                 $ ((Ck.checkIndex "Data/Vector/Generic/Mutable.hs" 630) Ck.Unsafe) "unsafeSwap" j (length v)
                 $ do
                     x <- unsafeRead v i
                     y <- unsafeRead v j
                     unsafeWrite v i y
...
move :: (PrimMonad m, MVector v a)
                => v (PrimState m) a -> v (PrimState m) a -> m ()
{-# INLINE move #-}
move dst src = ((Ck.check "Data/Vector/Generic/Mutable.hs" 677) Ck.Bounds) "move" "length mismatch"
                                          (length dst == length src)
             $ unsafeMove dst src
...
partitionMax f s n
  = do
      v <- ((Ck.checkLength "Data/Vector/Generic/Mutable.hs" 838) Ck.Internal) "unstablePartitionMax" n
         $ unsafeNew n
...
partitionUnknown f s
  = do
      ...
      ((Ck.checkSlice "Data/Vector/Generic/Mutable.hs" 868) Ck.Internal) "partitionUnknown" 0 n1 (length v1')
